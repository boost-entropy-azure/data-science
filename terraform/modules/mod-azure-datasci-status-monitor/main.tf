...
resource "null_resource" "uploadfile" {
  ...
  provisioner "local-exec" {
    ...
    command = "az storage file upload --share-name ${azurerm_storage_share.prometheus_config.name} --account-name ${var.storage_account_name} --account-key ${var.storage_account_key} --source ${local_file.prometheus_config_file.filename};rm -f ${local_file.prometheus_config_file.filename}"
  }
}
