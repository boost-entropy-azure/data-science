...
inputs = {
  ...
  namespace_name      = join("-", [yamldecode(file("${find_in_parent_folders("tg_environment_inputs.yml")}"))["cluster_name"], yamldecode(file("${find_in_parent_folders("tg_environment_inputs.yml")}"))["environment"], "mqtt-eventhubs"])
